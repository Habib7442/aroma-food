      ...
      {error ? (
        ...
        <>
          {(cuisines ?? []).length === 0 ? (
            ...
            // last page (e.g. a bookmarked/typed ?page=N after cuisines
            // were deleted). Pagination below still renders (it's outside
            // this branch) so there's a way back to a real page instead of
            // a dead end that looks like "no cuisines" when some exist.
            <p className="mt-6 text-sm text-primary-dark">No cuisines on this page.</p>
          ) : (
